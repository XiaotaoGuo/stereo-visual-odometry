...
KittiImagesReader::KittiImagesReader(const std::string& data_root_path,
    ...
    : data_root_path_(data_root_path),
      ...
      index_(index) {
    fs::path left_image_path = fs::path(data_root_path_) / left_folder_;
    fs::path right_image_path = fs::path(data_root_path_) / right_folder_;
    if (!fs::exists(left_image_path) || !fs::exists(right_image_path)) {
        std::stringstream ss;
        ss << "Couldn't find: " << left_image_path.c_str() << " or "
           << right_image_path.c_str() << std::endl;
        throw std::runtime_error(ss.str());
    }

    boost::format fmt("%06d.png");
    int8_t curr_idx = index_;
    int8_t num_images = 0;

    fs::path left_img_path = left_image_path / (fmt % curr_idx).str();
    fs::path right_img_path = right_image_path / (fmt % curr_idx).str();

    while (fs::is_regular_file(left_image_path) &&
           fs::is_regular_file(right_image_path)) {
        num_images++;
        curr_idx++;
        left_img_path = left_image_path / (fmt % curr_idx).str();
        right_img_path = right_image_path / (fmt % curr_idx).str();
    }
